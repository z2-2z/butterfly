...
mod tests {
    ...
    use libafl_bolts::prelude::{StdRand, current_nanos, Rand, HasLen};
    ...
    fn fuzz_mutators() {
        ...
        loop {
            ...
            for _ in 0..1000 {
                let mutation = rand.between(0, 17);
                ...
                if mutated {
                    if stream.len() > MAX_LEN {
                        panic!("Mutation #{mutation} went out of bounds for number of tokens");
                    }
                    
                    for token in stream.tokens() {
                        if !token.verify() {
                            panic!("Mutation #{mutation} produced invalid token: {token:?} (full stream: {stream:?})");
                        }
                    }
                }
            }
    ...
    fn test_generation() {
        ...
        stream.serialize_content(&mut buffer);
